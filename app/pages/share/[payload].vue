...
import { Step, TaxTipMode, type Check } from "~/types/check";
...
function computeTotals(data: SharedCheckPayload): Record<string, number> {
  ...
  if (data.taxTipMode === TaxTipMode.Equal) {
    const extraPerGuest =
      data.guests.length > 0 ? (data.tax + data.tip) / data.guests.length : 0;
